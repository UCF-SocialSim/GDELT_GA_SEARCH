...
public class EcosystemGenerationLoopIterator extends GDELT_Query_Ecosystem {
	
	int generations;
	int currentGeneration = 0;
	
	
	public EcosystemGenerationLoopIterator(int numGenerations, int populationSize, 
			int numSurvivorsEachGeneration, GDELT_Query ... queries) {
		super(populationSize, numSurvivorsEachGeneration, queries);
		generations = numGenerations;
	}

	@Override
	public GDELT_Query nextQuery() {
		GDELT_Query ret = super.nextQuery();
		if(ret == null) {
			currentGeneration++;
			if(currentGeneration == generations) return null;
			else{
				evolve();
				ret = next();
			}
	...
	public int countOfTotalIterations() {
		return generations * super.countOfTotalIterations();
	}
