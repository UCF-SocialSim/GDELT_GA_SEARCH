 ...
 * using mean to normalize.
 */
public class Scorer_NRMSE_Mean extends Scorer_RMSE{

	public Scorer_NRMSE_Mean() {
		super();
	}
	
	public Scorer_NRMSE_Mean(TimeSeries dataSet, int tMin, int tMax, LocalDateTime ... dateTimes) {
		super(dataSet, tMin, tMax, dateTimes);
	}
	
	@Override
	protected double normalize(double valToNormalize, double[] testvals) {
		double mean = mean(testvals);
		return mean != 0 ? valToNormalize/mean: valToNormalize;
	}
	
	public String getType() {
		return "Scorer_NRMSE_Mean";
	}
